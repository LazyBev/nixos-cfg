...
in {
  ...
  environment.systemPackages = with pkgs; [
    alejandra nil statix nix-output-monitor nix-init nurl nh
    adwaita-qt dracula-theme dracula-icon-theme grayjay wl-clipboard helix pkgit wl-mirror
  ];
  ...
  hjem.users.yari = {
    ...
    packages = with pkgs; [
      librewolf-wrapped librewolf-i2p impala-nm fetch
      opentabletdriver
      tinycc clang gcc wget
      ncurses
      gcc ghc go lua rustc rustfmt rustup cargo zig odin ocaml
      nasm fasm asm-lsp cmake gnumake flex bison byacc
      vscodium zathura
      devenv raylib help2man pkg-config opencode gh
      (python3.withPackages (ps: [ ps.pip ps.psutil ps.textual ]))
      qutebrowser
      signal-desktop vesktop gajim thunderbird briar-desktop
      protonmail-desktop weechat catgirl ii
      mpv mpvScripts.modernz mpvScripts.thumbfast pavucontrol playerctl sox ffmpeg imv gpu-screen-recorder
      wf-recorder slurp osu-lazer-bin waypaper swaybg mpvpaper rmpc
      alacritty bemenu dunst hypridle hyprlock networkmanagerapplet ribbon nemo
      motrix qbittorrent
      bat eza zoxide fzf ripgrep rlwrap zellij yazi
      btop ncdu tree unar unzip zip gnused gawk gnugrep which
      pciutils usbutils brightnessctl wget curl rsync pastel
      dysk astroterm caligula libnotify killall
      htop fastfetch strace ltrace inotify-tools
      iw bind dnsutils ucspi-tcp openssl
      proton-vpn proton-vpn-cli yt-dlp
      ncurses ncurses5 xwayland-satellite catppuccin-cursors just
      tor tor-browser arti torsocks proxychains
      hashcat clamav keepassxc monero-cli monero-gui xmrig
      android-tools libpcap bitwarden-desktop
    ];
    ...
    files = {
      ...
      ".config/librewolf/librewolf/rlubfwj2.default/chrome/userChrome.css".source = ../dotfiles/librewolf/userChrome.css;
      ...
      ".torrc".source = ../dotfiles/tor/torrc;
      ".config/mpv/mpv.conf".text = ''
        ...
        profile=gpu-hq
      '';
      ".config/mpv/scripts/modernz.lua".source = "${pkgs.mpvScripts.modernz}/share/mpv/scripts/modernz.lua";
      ".config/mpv/scripts/thumbfast.lua".source = "${pkgs.mpvScripts.thumbfast}/share/mpv/scripts/thumbfast.lua";
